...
\textbf {Southbound Interfaces} which main role is to connect the network devices between the controllers and network via protocols like OpenFlow (see section
\ref{sec:of}), or P4 \footnote {https://p4.org/}. 
...
These requirements are general principles for networks, but the addition of the SDN controller introduces another point of failure, that could be damaging to the 
entire network.
...
Since 2011, this protocol has suffered some revisions, and the latest version supported is version 1.5.1. 
...
that closed or commercial solutions give the network researchers.
...
which lives in a separate plane, all the switch needs to do is correctly match the incoming packets, and forward them according to the rules established by the
...
\begin{enumerate} 
    \item \textbf{General Purpose Processors} provide the greatest flexibility of all the solutions, while providing the worst results in performance, due to the
        general purpose design of the hardware and the optimizations present in the other architectures.
    \item \textbf{Field-Programmable gate arrays (FPGA)} are a platform that enable the configuration of the devices via hardware design tools, maintaining the
        ...
        research, and supporting up to 100G operation. 
...
the controllers, by having multiple backups running. In order to ensure that the same network state in every controller, every switch must be connected to 
...
comparative study is performed on available SDN controllers at the time, and compare the different characteristics of each controller, like the available
...
OpenDaylight \footnote{https://www.opendaylight.org/} (ODL) is a major project supported by the major vendors. The main differences between ODL and other controllers
...
combination with a RESTCONF interface, defining the data models for the data stores, and the RPCs for interaction between the data.
